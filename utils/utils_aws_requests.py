...
def vulnerable_cloudfront_s3_alias(account_id, account_name, domain):
    ...
    try:
        ...
        if response.status_code == 404 and "<Code>NotFound</Code>" in response.text:
            bucket_url = get_cloudfront_origin(account_id, account_name, domain)
...
def vulnerable_cloudfront_s3_cname(account_id, account_name, domain):
    try:
        response = requests.get(f"https://{domain}", timeout=1)
