resource "aws_cloudwatch_event_rule" "scheduled_event" {
  ...
  schedule_expression = contains(var.update_lambdas, trim(trimsuffix(trimprefix(each.value, var.project), var.environment), "-")) ? "rate(${var.update_schedule})" : "rate(${var.schedule})"
}
