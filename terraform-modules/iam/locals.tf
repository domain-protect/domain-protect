locals {
  ...
  dynamodb_table_name    = "${replace(title(replace(var.project, "-", " ")), " ", "")}VulnerableDomains${title(local.env)}"
  dynamodb_ip_table_name = "${replace(title(replace(var.project, "-", " ")), " ", "")}IPs${title(local.env)}"
