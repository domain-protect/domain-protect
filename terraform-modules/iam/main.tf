...
resource "aws_iam_role_policy" "lambda" {
  ...
  policy = templatefile("${path.module}/templates/${var.policy}_policy.json.tpl", { security_audit_role_name = var.security_audit_role_name, project = var.project, env = var.env, kms_arn = var.kms_arn, ddb_table_arn = local.dynamodb_arn, ddb_ip_table_arn = local.dynamodb_ip_arn, state_machine_arn = var.state_machine_arn })
}
